...
SUBMISSION_DIR = PROJECT_ROOT / "output"
...
def next_submission_path(stem: str = "submission", suffix: str = ".csv") -> Path:
    """Return the first unused submission_NN.csv so reruns never clobber each other."""
...
def write_submission(ids, preds, path: Path | None = None) -> Path:
    ...
    # dont trust that our own dataframe order matches theirs.
